//  Written by Mohamed Negm
//	Student number: 400267484
//  April 15 ,2021
//  Description: This code uses the time of flight sensor to take distance measurements at a specified angle
...
int status=0;
int flashIndex = 0;				//This variable is used for flashing the onboard LED at 45 degrees
  ...
  status = VL53L1X_StartRanging(dev);   /* This function has to be called to enable the ranging */
	//Status_Check("StartRanging", status);
		...
		for(int j=0;j<512;j++){
			...
			//Rotate the motor and Turn on LED D1
			int delay=1;
		...
		//Return the motor to it's original position
		for(int j=0;j<512;j++){
			GPIO_PORTL_DATA_R = 0b00001001;
			SysTick_Wait10ms(1);
			GPIO_PORTL_DATA_R = 0b00000011;
...
void cw_spin(){
	for(int i=0; i<512; i++){									//While the speed is not zero, continue spinning
		GPIO_PORTL_DATA_R = 0b00001100;
		SysTick_Wait10ms(1);
		GPIO_PORTL_DATA_R = 0b00000110;
		SysTick_Wait10ms(1);
		...
		GPIO_PORTL_DATA_R = 0b00001001;
		SysTick_Wait10ms(1);
		flashIndex += 4;												//Increment the flashIndex for each step
		flashLED();															//Call flashLED to check if the number of steps reached matches those corresponding to the angle input
	}	
}

void flashLED(){
					//Determine the number of steps required for 1 flash
	if(flashIndex == 256){						//If the flashIndex reaches the number of steps, output 1 to LED 1
		GPIO_PORTF_DATA_R ^= 0b00010000;
		flashIndex=0;
	}
	SysTick_Wait10ms(30);
	GPIO_PORTF_DATA_R &= 0b00000001;				//Wait for 30ms and turn the LED back off
}
